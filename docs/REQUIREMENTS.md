...
| D24 | **Doc round-trip — `docs/ARCHITECTURE.md` references the 5 NotebookLM features we transfer** | ✓ **Shipped 2026-07-13**: `docs/ARCHITECTURE.md` written. E2E probe verifies it exists and references each transferred feature (transcript-only ingest, source select per query, evidence-span IDs, inline citations, refusal on no-evidence). |
| D25 | **Job queue + daemon + Kanban** (rung 1 of the personal-ops platform) | ✓ **Shipped 2026-07-13**: `bin/jobs.py` (SQLite queue + audit log, idempotent on `key_hash`), `bin/daemon.py` (periodic dispatcher with stale-row reclamation), `bin/kanban.py` (TTY Kanban with `--watch --interval N`). See [docs/ARCHITECTURE.md](ARCHITECTURE.md) for the substrate. |
| D26 | **URL-picker UI** (sidebar list, transcript preview, slugs[] payload) | ✓ **Shipped 2026-07-13**: `bin/web.py` exposes `/api/videos?tag=&mode=&outcome=`, `/api/transcripts/{slug}?chars=`, and a new `slugs: [str]` field on `/api/query` that triggers url-list mode. The chat UI sidebar shows every analyzed video with mode/tag/outcome/has_transcript. |
| D27 | **Hybrid dense + BM25 + RRF retrieval + FTS5 pinpoint search** | ✓ **Shipped 2026-07-14**: `vector_store.hybrid_search()` (Reciprocal Rank Fusion k=60 of dense top-k + BM25 top-k), `pinpoint_search()` (BM25-only on an exact phrase with `start_s`/`end_s` per hit), `chunks_fts` FTS5 virtual table (porter unicode61 tokenizer, backfilled on first `ensure_vec` call). Schema migration: `chunk_meta.start_s/end_s REAL` columns. Web route `GET /api/pinpoint?phrase=…` returns click-to-second YouTube URLs (`?t=<start_s>`). 11 e2e probes added. |
| D28 | **`--ingest-raw` transcript-only path** (no Gemini call; 40k-URL walk economics) | ✓ **Shipped 2026-07-14**: `analyze.py --ingest-raw` fetches the YouTube transcript directly via `youtube-transcript-api`, writes `.transcript.jsonl` (timestamped segments) + `.transcript.txt` (legacy sidecar), embeds locally via `upsert_chunks_with_timestamps`, records `outcome='ok-raw'`. `--force` re-embeds. The 40k-URL Takeout walk at free-tier quota (6–18 months for 4-shape path) is bounded only by YouTube's rate limit (~200 req/min) when walking `--ingest-raw` — hours, not months. Pinpoint YouTube URLs work end-to-end for chunks ingested via this path. |
| D29 | **Agent loop on cron** ("review requirements, create tasks, complete them as a system designer") | ✓ **Shipped 2026-07-14**: `bin/agent_loop.py --once` reads `docs/REQUIREMENTS.md`, lists every unshipped D# with the trigger sentence, runs the e2e gate, exits. `--watch --interval 20m` is the cron-registered mode. Designed to be wrapped by `hermes --yolo --script bin/agent_loop.py` or any other self-prompting runner. The loop is *disciplined* (no invented rungs, no free-form LLM work — only surface the unshipped set + e2e status). Cron registration: `schtasks /Create /SC MINUTE /MO 20 /TN "video-pipeline-agent" /TR "C:\Users\karee\AppData\Local\hermes\hermes-agent\venv\Scripts\python.exe C:\Users\karee\projects\video-pipeline\bin\agent_loop.py"`. |
| D30 | **Repo-graph substrate (graphify) for context-bloat relief** | ✓ **Shipped 2026-07-16 on `work/graphify-integration`** (NOT YET MERGED to master — branch policy). `bin/agent_skill_graphify.py` wraps the [Graphify-Labs/graphify](https://github.com/Graphify-Labs/graphify) skill (PyPI: `graphifyy==0.9.16` — name has double-y to dodge a collision with the canonical `graphify` reservation). Subcommands: `rebuild` / `rebuild_if_stale` (local AST, no API key), `status`, `query "<q>"` (BFS), `path A B` (shortest), `explain X` (plain-language), `affected X` (reverse traversal). Each writes structured JSON to `graphify-out/agent-skill-results.json` (gitignored) so the next agent reads without re-invoking. Also installed as a Hermes skill at `~/.hermes/skills/graphify/SKILL.md` via `python -m graphify install --platform hermes`. After running on the repo: 240 nodes, 413 links, 13 communities. **2026-07-16 user correction:** my first memo conflated `agent0ai/dox` (the canonical, 1,276★ MIT framework the user actually named) with `0xadvait/doxa` (an unrelated 6★ belief-base tool). The D31 row is the corrected DOX integration. |
| D31 | **DOX (agent0ai/dox) — AGENTS.md hierarchy for context-bloat relief + agent discipline** | ✓ **Shipped 2026-07-16 on `work/dox-implementation`** (NOT YET MERGED to master — branch policy). Three AGENTS.md files: root (`AGENTS.md` — DOX rail, project-wide rules, child index), `bin/AGENTS.md` (script-writing contract), `docs/AGENTS.md` (doc-writing contract). Adapted from [agent0ai/dox](https://github.com/agent0ai/dox) (MIT, 1,276★ as of 2026-07-16). DOX is *not* a runtime framework — it's a single 3.9 KB `AGENTS.md` template + a discipline. The integration is markdown only: zero new code, zero new deps, zero new e2e probes of the discipline itself (the "read before editing" rule is behavioral, not testable). What IS testable: that the three AGENTS.md files exist, that the root file's Child DOX Index lists both children, and that `docs/AGENTS.md` names the four existing docs. 8 e2e probes added. **Complementary to D30, not competitive:** graphify = *structure* (where is X, what does Y impact); DOX = *contract* (what are the local rules for editing this folder). Together they give the next agent a complete picture: code structure + project rules. Memo: `~/Documents/notes/research/agent0ai-dox-evaluation-2026-07-16.md`. |
| D32 | **Takeout walker — `bin/ingest_takeout.py` for the 40k-URL drain** | ✓ **Shipped 2026-07-16 on `work/takeout-full-walk`** (NOT YET MERGED to master — branch policy). User's exact pain: "make sure all videos from takeout are analyzed rather than just stopping at a few." The walker scans every `takeout-*.zip` in `~/Downloads/`, dedupes URLs against the corpus index, and enqueues not-yet-analyzed URLs into `jobs.sqlite` with `mode=ingest-raw` (no Gemini). Three subcommands: `status` (per-zip URL count + format detection — JSON vs HTML), `enqueue` (idempotent on `key_hash`), `walk` (enqueue + dispatch in one shell, optional `--rate` flag for YouTube caption API pacing). Status surfaces a real data drift bug: 4 of the 6 zips are pre-2024 HTML format, not JSON — the walker reports `[format: html]` and warns the user. **`bin/jobs.py` `_worker_analyze` was extended to dispatch `mode=ingest-raw` → `analyze.py --ingest-raw --force`**; existing `mode=transcript` and `mode=multimodal` paths preserved. **40,887 URLs not yet analyzed** across the 2 JSON zips; the rest are HTML-only (older Takeout format, not parsed). Stdlib only; 3 e2e probes added. **Drain command** (user invokes when ready): `python bin/ingest_takeout.py enqueue && python bin/jobs.py daemon --interval 20m` — runs unattended until the queue drains or the user interrupts. |
